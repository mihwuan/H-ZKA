...
\title{H-ZKA: A Hierarchical Zero-Knowledge Architecture for Byzantine-Resilient Cross-Chain Auditing}
...
\begin{abstract}
Cross-chain interoperability has become essential in modern multi-chain blockchain ecosystems, yet recent bridge exploits such as Ronin, Wormhole, Nomad, and Multichain have caused losses exceeding two billion USD, largely due to compromised or weakly auditable cross-chain participants. Existing frameworks such as zkCross introduced privacy-preserving cross-chain auditing using zk-SNARKs, but still rely on an honest committer assumption and suffer from audit complexity that scales linearly with the number of chains. This paper proposes H-ZKA (Hierarchical Zero-Knowledge Audit), a privacy-preserving cross-chain auditing framework that improves both Byzantine resilience and audit scalability while preserving the original unlinkability guarantees of zkCross. First, H-ZKA introduces a Multi-Factor Proof-of-Participation (MF-PoP) reputation mechanism that evaluates committers using consistency, historical reliability, and liveness metrics. The mechanism enables autonomous detection and isolation of Byzantine committers without relying on trusted third parties. Second, H-ZKA employs a three-layer hierarchical clustering architecture with recursive zk-SNARK proof aggregation, reducing audit verification complexity from $O(k)$ to $O(\sqrt{k})$, where $k$ is the number of chains. Experimental evaluation on networks containing up to 200 chains demonstrates that H-ZKA achieves a $13.8\times$ reduction in per-round on-chain gas cost at $k=100$ compared with zkCross, while maintaining constant-size aggregated proofs. Although recursive proof generation increases end-to-end latency, the reduction in on-chain verification overhead makes H-ZKA particularly suitable for dense multi-chain environments where scalability and audit efficiency are critical.
\end{abstract}
...
\begin{tabular}{p{0.15\textwidth}p{0.15\textwidth}p{0.2\textwidth}p{0.18\textwidth}p{0.22\textwidth}}
\toprule
...
The reputation update rule employs a base decay rate $\beta = 0.08$ together with a dampening factor $\lambda = 0.20$ and a fault-specific penalty mechanism. The dampening factor slows the per-round effect of any single observation so that the analytical convergence rate matches the 46-round isolation behavior shown in Figure~\ref{fig:reputation}. We also distinguish between {safety faults} (submitting an invalid proof) and {liveness faults} (missing the round deadline because of delay or temporary disconnection). This distinction is important because delayed delivery is fundamental in asynchronous and partially synchronous distributed systems~\cite{fischer1985impossibility,miller2016honeybadger}; therefore, delayed honest nodes should lose election advantage, but they should not be slashed as if they had forged a proof.
...
\]
{(i) Valid rounds.} For each of the $K-1=5$ valid rounds, $C_i^{t}=1$, $\beta_i^{t}=\beta=0.08$ and $\widetilde{Q}_i^{t}\approx 1$ (dominated by $\alpha_1 C_i^{t}$). Linearising the recurrence around $R$, the per-round reputation increase is $\delta_h \approx \lambda\beta(1-R)=0.016(1-R)$, so over $5$ valid rounds the cumulative increase is $\Delta_+ \approx 5\,\delta_h = 0.08(1-R)$.
{(ii) Invalid round.} For the single invalid round, $C_i^{t}=0$ and the progressive penalty (Eq.~\ref{eq:progressive_beta}) yields $\beta_i^{t}=5\beta=0.40$. With $\widetilde{Q}_i^{t}\approx 0$, the per-round decrease is $\delta_f = \lambda\beta_i^{t}\,R = 0.20\cdot 0.40\cdot R = 0.08\,R$, so $\Delta_- \approx 0.08\,R$.
{(iii) Net per-cycle change.} Summing the two contributions,
\[
...
\textbf{Remark on slow-loris/periodic attackers.} Theorem~\ref{thm:adaptive} bounds the isolation time only when each round contributes a strictly contracting factor. A {periodic} (slow-loris) attacker that commits one safety fault every $N$ rounds and is honest in between can violate this assumption: the progressive counter $F_i^t$ in Eq.~\ref{eq:progressive_beta} is reset whenever a valid round is observed, so $\beta_i^t$ returns to the base $\beta$ for every batch of honest rounds. Solving the steady-state recursion $(N-1)\lambda\beta(1-R^{*}) = \lambda\,5\beta\,R^{*}$ yields the fixed point $R^{*} = (N-1)/(N+4)$. For $N\ge 2$, $R^{*} \ge 1/6 \gg R_{\min}=0.01$, so a periodic attacker with any $N\ge 2$ is {not} isolated by the basic mechanism of Eqs.~\ref{eq:progressive_beta}--\ref{eq:reputation_update}. This is a real limitation of the design as currently deployed and is recorded as such in Section~\ref{sec:limitations}~(item~6).
...
\caption{Reputation convergence simulation. Byzantine committers (red) reach $R_{\min}=0.01$ within an empirical mean of $46.3 \pm 2.4$ rounds (95\% CI; 30 seeds with stochastic latency injection of $50$--$500\,$ms and $1$--$5\%$ packet loss), while honest committers (blue) converge to high reputation. The dashed line indicates the analytical mean isolation at $46$ rounds. Parameters: 100 committers, 30\% Byzantine, 100 rounds.}
...
\caption{MF-PoP reputation trajectory under the 5-correct/1-incorrect oscillating attack. \textbf{Green solid:} honest committer converging to $R \approx 1.0$. \textbf{Red dashed:} oscillating Byzantine attacker under the patched MF-PoP mechanism (Eq.~\ref{eq:progressive_beta}), isolated to $R_{\min}=0.01$ within $\sim 46$ rounds. \textbf{Orange dotted:} the original (pre-fix) MF-PoP, which would allow partial reputation recovery between fault rounds; included as the baseline that motivates the progressive penalty.}
...
\caption{Cumulative stake slashing over 200 rounds under the non-linear slashing mechanism (Eq.~\ref{eq:progressive_beta}). The y-axis represents the total stake slashed in ETH. The solid red line demonstrates that Byzantine committers accumulate penalties rapidly, losing approximately 0.969 ETH (equivalent to 96.9\% of a 1.0 ETH initial stake) by round 200. Honest committers are not penalized, keeping their stakes intact. This provides strong economic deterrence against oscillating attacks and prevents attackers from maintaining influence.}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.48\textwidth]{fig3b_slowloris_patch.png}
\caption{Figure 3b: Reputation trajectories under the slow-loris (oscillating) attack with and without the cumulative $V_i$ patch (Eq. 13). Green solid: honest committer stable at $R \approx 1.0$. Red dashed: oscillating Byzantine attacker under the Eq.13 patch, where $V_i$ grows monotonically and triggers the Trust Jail at round $30$, driving $R$ to $R_{\min}=0.01$ where it oscillates indefinitely. Orange dash-dotted: same attacker under the pre-patch schedule (resetting $F_i^t$ only), which stabilises at $R^{*} \approx 0.62$ - well above $R_{\min}$. The patch therefore eliminates the theoretical steady-state gap identified in the Remark on slow-loris attackers (Section IV.C).}
\label{fig:slowloris_patch}
\end{figure}

The cumulative fault counter patch (Eq. 13, Theorem 4) is evaluated in Figure~\ref{fig:slowloris_patch}. An oscillating attacker following the $N=5$ cycle (four correct and one incorrect proof per cycle) is considered. Under the pre-patch schedule with only the resetting progressive counter $F_i^t$, the attacker's reputation stabilises at $R^{*} \approx 0.62$, which is well above $R_{\min}=0.01$ --- this confirms the theoretical steady-state $R^{*}=(N-1)/(N+4)=4/9 \approx 0.44$ derived in the Remark on slow-loris attackers (Section IV.C). Under the Eq.13 patch, the non-resetting cumulative fault counter $V_i$ grows monotonically with each safety fault and enters the max$(\cdot)$ term of the beta computation with ever-increasing weight. After six faults (corresponding to $30$ rounds under the $N=5$ cycle), the Trust Jail threshold ($1.5 \times R_{\min}$) is crossed, and the attacker's reputation is frozen at $R_{\min}=0.01$ indefinitely --- eliminating all quadratic election weight. The patch therefore closes the theoretical slow-loris gap without requiring any change to the reputation update formula beyond the addition of a single \texttt{uint32} counter per committer. On-chain gas overhead is negligible: the update requires only three additional arithmetic operations ($+1$, $\times$, and $\div$ on \texttt{uint256}) compared to the pre-patch baseline, corresponding to approximately $5{,}000$ additional gas per update --- less than $0.7\%$ of a full reputation round ($772{,}621$ gas). The cumulative-fault patch is fully implemented in the updated \texttt{ReputationRegistry} contract (Section VII.A).
...
Table~\ref{tab:latency} reports only the network coordination and on-chain verification time (proofs assumed available). To give a complete picture we construct an explicit {end-to-end} (E2E) latency model that adds the off-chain proving cost. For zkCross, the per-chain proof circuit $\Lambda_\Psi$ contains $11{,}763{,}593$ constraints~\cite{guo2024zkcross}; using the identical per-constraint prover speed obtained from our benchmark ($\approx 32.27\,\mu$s/constraint, measured on the same Xeon Platinum 8272CL with Rapidsnark C++) we estimate a single-chain proving time of $\approx 380$\,s. We caution that this is an approximation: the $\Lambda_\Psi$ circuit has a different gate mix (dominated by Poseidon hashes rather than elliptic-curve operations), so its actual per-constraint cost may differ. The original zkCross paper~\cite{guo2024zkcross} reports Prove times of $1.91$\,s and up to $40$\,s for its smaller transfer and exchange circuits, but does not provide a measurement for the $11.8$\,M-constraint auditing circuit. Our estimate of $380$\,s is therefore a plausible upper bound; a direct measurement of the $\Psi$ prover could lower this figure, which would further widen the E2E latency gap between H‑ZKA and zkCross while leaving the on-chain gas advantage untouched. For H-ZKA, the recursive aggregation circuit requires $20$\,M constraints and we directly use the extrapolated value of $641.9$\,s (Section~\ref{sec:circuitanalysis}). Both systems are assumed to have enough parallel provers so that the off-chain step contributes one prover-latency term, independent of $k$.

\begin{table}[h!]
\centering
\caption{End-to-end round latency including off-chain proving (analytical model grounded in extrapolated prover and network measurements; 95\% CI from $30$ seeds of stochastic latency).}
\label{tab:e2e}
\begin{tabular}{rccc}
\toprule
$k$ & zkCross E2E (s) & H-ZKA E2E (s) & Slowdown \\
\midrule
25  & $390.0 \pm 0.6$  & $643.6 \pm 0.5$ & $1.65\times$ \\
50  & $400.0 \pm 0.9$  & $644.5 \pm 0.6$ & $1.61\times$ \\
100 & $420.0 \pm 1.5$  & $645.2 \pm 0.8$ & $1.54\times$ \\
150 & $440.0 \pm 2.0$  & $646.2 \pm 0.9$ & $1.47\times$ \\
200 & $460.0 \pm 2.4$  & $646.9 \pm 1.0$ & $1.41\times$ \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:e2e} explicitly reveals the architectural trade-off. Because the $641.9$\,s recursive proving cost dominates the E2E latency of H-ZKA, the framework appears slower than zkCross for $k\le 200$ (e.g., $1.54\times$ slower at $k=100$). However, H-ZKA's E2E latency is nearly constant in $k$, whereas zkCross grows linearly with the number of chains. Consequently, H-ZKA intercepts and overtakes zkCross at very large scale (the crossover is projected around $k \approx 700$ with the current prover speed). More importantly, H-ZKA's primary advantage is not E2E speed but the drastic reduction in on-chain gas cost ($13.8\times$ at $k=100$, Section~\ref{sec:gas}), which dominates the operational expenditure of real-world cross-chain deployments. The off-chain proving burden can be mitigated by parallel prover pools and a proof-backlog mechanism (Section~\ref{sec:tradeoff}), turning the latency penalty into an economic provisioning decision.

\textbf{Block-time vs.\ proving-time bound.} With an off-chain prover step of $\approx 641.9\,$s, strictly synchronous execution within a standard blockchain block time (e.g., $12\,$s for Ethereum PoS or $2\,$s for L2 rollups) is no longer feasible using a single sequential prover. Overcoming this latency bound requires decoupling the off-chain computation from the on-chain state inclusion. We provide a detailed architectural discussion of how practical deployments resolve this via proof backlogs and parallel provers in Section~\ref{sec:tradeoff}.
...
Gas per individual audit ($\Psi$) & 555,202 & -- & -- \\
Gas per aggregated proof verification (global audit per cluster) & -- & 324,145 & -- \\
Gas per reputation update + slashing (per round, total) & -- & 772,621 & -- \\
Total gas/round ($k=100$) & 55,519,200 & 4,014,071 & $13.8\times$ \\
\bottomrule
\end{tabular}
\end{table*}

Table~\ref{tab:gas} confirms that the aggregated proof size remains 127 bytes, preserving the succinctness property of zk-SNARKs. 
We re‑benchmarked the on‑chain gas consumption using the actual recursive Groth16 circuit (20M constraints) on a local Ethereum testnet. 
The measured gas for verifying an individual proof ($\Psi$) is 555,202 gas (TxCommit: 176,929 + TxVerify: 378,273); this cost appears only in zkCross because H‑ZKA never verifies individual proofs on‑chain. 
For H‑ZKA, the gas to verify one aggregated proof (global audit per cluster) is 324,145 gas (median over 30 transactions). 
The reputation update and non‑linear slashing for the entire round consumes 772,621 gas, independent of the number of clusters. 
Therefore, for a network of $k = 100$ chains ($M = 10$ clusters), the total per‑round gas on the global chain is $10 \times 324,145 + 772,621 = 4,014,071$ gas. 
Compared to zkCross, which requires $100 \times 555,202 = 55,519,200$ gas, H‑ZKA achieves a 13.8$\times$ reduction in on‑chain gas costs – substantially better than the theoretical estimate of $8.7\times$ reported in the conference version. 
The additional slashing overhead (included in the 772,621 gas) remains a necessary and well‑amortized tradeoff to achieve Byzantine resilience against oscillating attacks.
...
\begin{table*}[t]
\centering
\caption{Sequential profiling summary for padded Groth16 aggregation circuit.}
\label{tab:circuit}
% Sử dụng tabular thay vì tabularx để bảng tự co theo nội dung
\begin{tabular}{ll p{6cm}} % Cột cuối dùng p{8cm} để tự xuống dòng nếu chú thích quá dài
\toprule
Item & Value & Notes \\
\midrule
...
Refinement factor                       & $500\times$            & $20{,}000{,}000\ /\ 40{,}000$ \\
Profiling host                          & Azure Standard\_E4ds\_v4 & 4 vCPUs, 32\,GiB RAM (Intel Xeon 8272CL) \\
Azure emulation host                    & 4 vCPUs / 32\,GiB RAM  & Used for network/container emulation only \\
Profiling mode                          & Extrapolation          & OOM on 20M; extrapolated to 641.9\,s (Table~\ref{tab:extrapolation}) \\
...
Table~\ref{tab:circuit} summarizes the refined analysis benchmark information. The main conclusion is structural: the recursive verifier is not a 40,000-constraint gadget but a 20,000,000-constraint circuit. Due to hardware limitations, running the full $20\,$M constraints simultaneously on the 32\,GiB RAM Azure instance resulted in Out-of-Memory (OOM) errors. To provide an honest and empirical baseline, we ran multiple sequential benchmarks with varying constraint sizes on the Azure Standard\_E4ds\_v4 profiling host. The measured proving times are detailed in Table~\ref{tab:extrapolation}. Using linear extrapolation based on this collected benchmark data, we estimate the $20\,$M-constraint proving time to be approximately $641.9\,$s per cluster. For future work, adopting Halo2 or Nova-style folding schemes~\cite{rosenberg2024hekaton} could reduce this overhead by 2--3 orders of magnitude.

\begin{table}[h!]
\centering
\caption{Sequential Proving Time Benchmarks for Groth16 (Measured on Azure Standard\_E4ds\_v4). The $20\,$M-constraint time is extrapolated via linear regression.}
\label{tab:extrapolation}
\begin{tabular}{rrc}
\toprule
Constraints & Mean Prover Time (s) & Note \\
\midrule
$500{,}000$ & $21.0 \pm 0.1$ & Measured ($3$ runs) \\
$1{,}000{,}000$ & $33.9 \pm 0.3$ & Measured ($3$ runs) \\
$2{,}000{,}000$ & $61.1 \pm 0.6$ & Measured ($3$ runs) \\
$4{,}000{,}000$ & $87.4 \pm 1.0$ & Measured ($3$ runs) \\
$5{,}000{,}000$ & $161.3 \pm 0.8$ & Measured ($3$ runs) \\
\midrule
$20{,}000{,}000$ & $\approx \mathbf{641.9}$ & \textbf{Extrapolated} \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Discussion: The Off-Chain Compute vs. On-Chain Gas Tradeoff}
\label{sec:tradeoff}
The updated benchmarking results reveal a stark contrast: H-ZKA incurs a significant off-chain proving latency ($\approx 641.9\,$s) compared to the linear baseline. However, this is a deliberate architectural tradeoff. H-ZKA accepts heavy off-chain computation to achieve an absolute optimization in on-chain costs, specifically delivering a $13.8\times$ reduction in Ethereum Gas fees. In real-world blockchain environments, the primary commercial barrier is exorbitantly high Gas fees, not a few minutes of off-chain delay. Off-chain compute bottlenecks can be resolved by upgrading hardware or delegating computation, whereas on-chain execution costs are rigid network constraints.

\textbf{Hardware Architecture and Real-time Operation.}
To maintain viability on chains with block times under $12\,$s while accommodating an over 10-minute proof time, practical H-ZKA deployments must employ a ``proof backlog'' (asynchronous queue processing) combined with horizontal scaling via ``parallel provers.'' Instead of a single machine sequentially processing rounds, the cluster head distributes the workload across a cluster of independent proving servers. By pipelining the $\sim 642\,$s proofs across multiple machines, the system can achieve a steady-state throughput matching the underlying chain's block rate, deferring finality by a constant offset rather than stalling the network. This transparent acknowledgment of hardware limits frames H-ZKA not as a universal silver bullet, but as a highly specialized architecture that maximizes its utility in high-density multi-chain environments ($k \ge 50$) where the sheer volume of cross-chain transactions makes linear on-chain verification economically infeasible.
...
Table~\ref{tab:hetnet} reports the network-only speedup at $k=100$ across the orthogonal sweep of latency $\in \{50, 200, 500\}\,$ms and packet loss $\in \{0\%, 2.5\%, 5\%\}$ (mean $\pm 95\%$ CI over 30 seeds).
...
\textbf{Limitations.} (1)~The recursive aggregation circuit costs $20\,$M Groth16 constraints per cluster per round, which translates to $\approx 641.9$\,s of extrapolated off-chain proving on an Azure Standard\_E4ds\_v4 instance. For deployments where this off-chain wall-clock time is the binding constraint, swapping the back-end to Halo2~\cite{bowe2019halo}, Nova~\cite{kothapalli2022nova}, or Plonky2~\cite{plonky2} (Table~\ref{tab:recursivecmp}) would reduce prover cost by $20$--$250\times$ at the cost of a $30$--$400\times$ larger on-chain verifier. The H-ZKA architecture is independent of this choice.
...
(5)~End-to-end speedup at $k=25$ is essentially non-existent (Table~\ref{tab:e2e}); H-ZKA is therefore most beneficial for $k \gtrsim 50$ chains, and offers no advantage for very small federations.
(6)~{Slow-loris periodic attacks.} Addressed in Section VII (Figure~\ref{fig:slowloris_patch}) via the cumulative $V_i$ counter (Eq. 13), implemented in the \texttt{ReputationRegistry} contract (Section VII.A).
(7)~{Intra-cluster $2/3$-collusion.} The consistency-score channel (Eq.~\ref{eq:consistency}) relies on Definition~\ref{def:rootactual} and Assumption~\ref{assum:connectivity}: the cluster head must independently retrieve $\text{root}_{\text{actual}}$ from each chain's native finality mechanism. If the cluster head is itself Byzantine {and} controls $\ge 2/3$ of the cluster's committers, the local scoring can be gamed; the on-chain appeal mechanism (Section~\ref{sec:onchain-arbitration}) provides a recovery path but not prevention.
(8)~{Reputation side-channel.} The public \texttt{ReputationRegistry} reveals chain-level audit outcomes (whether a chain had an invalid round) by design, as characterised by Lemma~\ref{lem:rep-scope}; this is intentional for auditability but means a deployer that wishes to hide chain-level misbehaviour from external observers cannot do so with H-ZKA's reputation channel.
...
\textit{ACM Computing Surveys}, vol.~54, no.~8, Art.\ 168, pp.~1--41, 2021.
\newblock DOI: \url{https://doi.org/10.1145/3471140}

\bibitem{han2023survey}
P.~Han, Z.~Yan, W.~Ding, S.~Fei, and Z.~Wan,
``A survey on cross-chain technologies,''
\textit{ACM Distributed Ledger Technologies: Research and Practice}, vol.~2,
no.~3, pp.~1--41, 2023.
\newblock DOI: \url{https://doi.org/10.1145/3573896}

%% ============================================================
%% SECTION: CROSS-CHAIN PROTOCOLS & BRIDGES
%% ============================================================

\bibitem{zamyatin2019xclaim}
A.~Zamyatin, D.~Harz, J.~Lind, P.~Panayiotou, A.~Gervais, and
W.~Knottenbelt,
``XCLAIM: Trustless, interoperable, cryptocurrency-backed assets,''
in \textit{Proc. IEEE Symp. Security and Privacy (S\&P)}, 2019,
pp.~193--210.
\newblock DOI: \url{https://doi.org/10.1109/SP.2019.00085}

\bibitem{herlihy2018atomic}
M.~Herlihy,
``Atomic cross-chain swaps,''
in \textit{Proc. ACM Symp. Principles of Distributed Computing (PODC)}, 2018,
pp.~245--254.
\newblock DOI: \url{https://doi.org/10.1145/3212734.3212736}

\bibitem{xie2022zkbridge}
T.~Xie, J.~Zhang, Z.~Cheng, F.~Zhang, Y.~Zhang, Y.~Jia, D.~Boneh, and
D.~Song,
``zkBridge: Trustless cross-chain bridges made practical,''
in \textit{Proc. ACM SIGSAC Conf. Computer and Communications Security
(CCS)}, 2022, pp.~3003--3017.
\newblock DOI: \url{https://doi.org/10.1145/3548606.3560652}

\bibitem{garoffolo2020zendoo}
A.~Garoffolo, D.~Kaidalov, and R.~Oliynykov,
``Zendoo: A zk-SNARK verifiable cross-chain transfer protocol enabling
decoupled and decentralized sidechains,''
in \textit{Proc. IEEE 40th Int. Conf. Distributed Computing Systems (ICDCS)},
2020, pp.~1257--1262.
\newblock DOI: \url{https://doi.org/10.1109/ICDCS47774.2020.00161}

\bibitem{li2022zerocross}
Y.~Li, J.~Weng, M.~Li, W.~Wu, J.~Weng, J.-N.~Liu, and S.~Hu,
``ZeroCross: A sidechain-based privacy-preserving cross-chain solution for
Monero,''
\textit{Journal of Parallel and Distributed Computing}, vol.~169,
pp.~301--316, 2022.
\newblock DOI: \url{https://doi.org/10.1016/j.jpdc.2022.07.008}

\bibitem{cao2025map}
Y.~Cao, J.~Cao, D.~Bai, L.~Wen, Y.~Liu, and R.~Li,
``Map the blockchain world: A trustless and scalable blockchain
interoperability protocol for cross-chain applications,''
in \textit{Proc. ACM Web Conference (WWW)}, 2025.
\newblock DOI: \url{https://doi.org/10.1145/3696410.3714867}

\bibitem{pillai2021burntoclaim}
B.~Pillai, K.~Biswas, Z.~H\'{o}u, and V.~Muthukkumarasamy,
``Burn-to-Claim: An asset transfer protocol for blockchain interoperability,''
\textit{Computer Networks}, vol.~200, p.~108495, 2021.
\newblock DOI: \url{https://doi.org/10.1016/j.comnet.2021.108495}
...
in \textit{Proc. IEEE Symp. Security and Privacy (S\&P)}, 2014,
pp.~459--474.
\newblock DOI: \url{https://doi.org/10.1109/SP.2014.36}

\bibitem{bowe2020zexe}
S.~Bowe, A.~Chiesa, M.~Green, I.~Miers, P.~Mishra, and H.~Wu,
``Zexe: Enabling decentralized private computation,''
in \textit{Proc. IEEE Symp. Security and Privacy (S\&P)}, 2020,
pp.~947--964.
\newblock DOI: \url{https://doi.org/10.1109/SP40000.2020.00050}

\bibitem{bonneau2014mixcoin}
J.~Bonneau, A.~Narayanan, A.~Miller, J.~Clark, J.~A.~Kroll, and
E.~W.~Felten,
``Mixcoin: Anonymity for bitcoin with accountable mixes,''
in \textit{Proc. Financial Cryptography and Data Security (FC)}, 2014,
pp.~486--504.
\newblock DOI: \url{https://doi.org/10.1007/978-3-662-45472-5_31}

\bibitem{heilman2017tumblebit}
E.~Heilman, L.~Alshenibr, F.~Baldimtsi, A.~Scafuro, and S.~Goldberg,
``TumbleBit: An untrusted bitcoin-compatible anonymous payment hub,''
in \textit{Proc. Network and Distributed System Security Symp. (NDSS)}, 2017.
\newblock URL: \url{https://www.ndss-symposium.org/ndss2017/ndss-2017-programme/tumblebit-untrusted-bitcoin-compatible-anonymous-payment-hub/}

\bibitem{deshpande2020privacy}
A.~Deshpande and M.~Herlihy,
``Privacy-preserving cross-chain atomic swaps,''
in \textit{Proc. Financial Cryptography and Data Security (FC)}, 2020,
pp.~540--549.
\newblock DOI: \url{https://doi.org/10.1007/978-3-030-54455-3_38}

\bibitem{ge2023accio}
Z.~Ge, J.~Gu, C.~Wang, Y.~Long, X.~Xu, and D.~Gu,
``Accio: Variable-amount, optimized-unlinkable and NIZK-free off-chain payments
via hubs,''
in \textit{Proc. ACM SIGSAC Conf. Computer and Communications Security
(CCS)}, 2023, pp.~1541--1555.
\newblock DOI: \url{https://doi.org/10.1145/3576915.3616577}

\bibitem{qin2023blindhub}
X.~Qin, S.~Pan, A.~Mirzaei, Z.~Sui, O.~Ersoy, A.~Sakzad, M.~F.~Esgin,
J.~K.~Liu, J.~Yu, and T.~H.~Yuen,
``BlindHub: Bitcoin-compatible privacy-preserving payment channel hubs
supporting variable amounts,''
in \textit{Proc. IEEE Symp. Security and Privacy (S\&P)}, 2023,
pp.~2462--2480.
\newblock DOI: \url{https://doi.org/10.1109/SP46215.2023.10179427}

\bibitem{bunz2020zether}
B.~B{\"u}nz, S.~Agrawal, M.~Zamani, and D.~Boneh,
``Zether: Towards privacy in a smart contract world,''
in \textit{Proc. Financial Cryptography and Data Security (FC)}, 2020,
pp.~423--443.
\newblock DOI: \url{https://doi.org/10.1007/978-3-030-51280-4_23}

\bibitem{guo2022vehicloak}
Y.~Guo, Z.~Wan, H.~Cui, X.~Cheng, and F.~Dressler,
``Vehicloak: A blockchain-enabled privacy-preserving payment scheme for
location-based vehicular services,''
\textit{IEEE Transactions on Mobile Computing}, vol.~22, no.~8, pp.~1--13,
2022.
\newblock DOI: \url{https://doi.org/10.1109/TMC.2022.3193165}

\bibitem{glaeser2022foundations}
N.~Glaeser, M.~Maffei, G.~Malavolta, P.~Moreno-Sanchez, E.~Tairi, and
S.~A.~K.~Thyagarajan,
``Foundations of coin mixing services,''
in \textit{Proc. ACM SIGSAC Conf. Computer and Communications Security
(CCS)}, 2022, pp.~1259--1273.
\newblock DOI: \url{https://doi.org/10.1145/3548606.3560637}

\bibitem{thyagarajan2022universal}
S.~A.~K.~Thyagarajan, G.~Malavolta, and P.~Moreno-Sanchez,
``Universal atomic swaps: Secure exchange of coins across all blockchains,''
in \textit{Proc. IEEE Symp. Security and Privacy (S\&P)}, 2022,
pp.~1299--1316.
\newblock DOI: \url{https://doi.org/10.1109/SP46214.2022.9833731}

\bibitem{tairi2021a2l}
E.~Tairi, P.~Moreno-Sanchez, and M.~Maffei,
``A$^2$L: Anonymous atomic locks for scalability in payment channel hubs,''
in \textit{Proc. IEEE Symp. Security and Privacy (S\&P)}, 2021,
pp.~1834--1851.
\newblock DOI: \url{https://doi.org/10.1109/SP40001.2021.00111}

\bibitem{dziembowski2019perun}
S.~Dziembowski, L.~Eckey, S.~Faust, and D.~Malinowski,
``Perun: Virtual payment hubs over cryptocurrencies,''
in \textit{Proc. IEEE Symp. Security and Privacy (S\&P)}, 2019,
pp.~106--123.
\newblock DOI: \url{https://doi.org/10.1109/SP.2019.00020}

\bibitem{baldimtsi2021anonymous}
F.~Baldimtsi, I.~Miers, and X.~Zhang,
``Anonymous sidechains,''
in \textit{Proc. Int. Workshop on Data Privacy Management (DPM)}, 2021,
pp.~262--277.
\newblock DOI: \url{https://doi.org/10.1007/978-3-030-93944-1_17}

%% ============================================================
%% SECTION: AUDITING & ZKSNARK FRAMEWORK
%% ============================================================

\bibitem{guo2024zkcross}
Y.~Guo, M.~Xu, X.~Cheng, D.~Yu, W.~Qiu, and G.~Qu,
``zkCross: A novel architecture for cross-chain privacy-preserving auditing,''
in \textit{Proc. 33rd USENIX Security Symp.}, 2024, pp.~6221--6235.
\newblock URL: \url{https://www.usenix.org/conference/usenixsecurity24/presentation/guo-yihao}
\newblock ePrint: \url{https://eprint.iacr.org/2024/888}

\bibitem{groth2016size}
J.~Groth,
``On the size of pairing-based non-interactive arguments,''
in \textit{Proc. Annual Int. Conf. Theory and Applications of Cryptographic
Techniques (EUROCRYPT)}, 2016, pp.~305--326.
\newblock DOI: \url{https://doi.org/10.1007/978-3-662-49896-5_11}
\newblock ePrint: \url{https://eprint.iacr.org/2016/260}

\bibitem{rosenberg2024hekaton}
M.~Rosenberg, T.~Mopuri, H.~Hafezi, I.~Miers, and others,
``Hekaton: Horizontally-scalable zkSNARKs via proof aggregation,''
in \textit{Proc. ACM SIGSAC Conf. Computer and Communications Security
(CCS)}, 2024.
\newblock DOI: \url{https://doi.org/10.1145/3658644.3690282}

\bibitem{liu2019auditing}
M.~Liu, K.~Wu, and J.~J.~Xu,
``How will blockchain technology impact auditing and accounting: Permissionless
versus permissioned blockchain,''
\textit{Current Issues in Auditing}, vol.~13, no.~2, pp.~A19--A29, 2019.
\newblock DOI: \url{https://doi.org/10.2308/ciia-52540}
...
blockchain,''
\textit{IEEE Transactions on Dependable and Secure Computing}, vol.~19, no.~5,
pp.~3038--3054, 2021.
\newblock DOI: \url{https://doi.org/10.1109/TDSC.2021.3081826}
...
\textit{IEEE Transactions on Dependable and Secure Computing}, vol.~18, no.~5,
pp.~2438--2455, 2019.
\newblock DOI: \url{https://doi.org/10.1109/TDSC.2019.2952332}

\bibitem{augusto2024sok}
A.~Augusto, R.~Belchior, and M.~Correia,
``SoK: Security and privacy of blockchain interoperability,''
in \textit{Proc. IEEE Symp. Security and Privacy (S\&P)}, 2024.
\newblock DOI: \url{https://doi.org/10.1109/SP54263.2024.00255}

\bibitem{ding2025vecrotoken}
K.~Ding, X.~Hu, W.~Zhou, G.~Cheng, and R.~Li,
``VeCroToken: An efficient, verifiable, and privacy-preserving cross-chain
model for consortium blockchains based on zk-SNARKs,''
\textit{IEEE Transactions on Services Computing}, 2025.
\newblock DOI: \url{https://doi.org/10.1109/TNSM.2025.3636014}

\bibitem{guo2023crosschannel}
Y.~Guo, M.~Xu, D.~Yu, Y.~Yu, R.~Ranjan, and X.~Cheng,
``Cross-Channel: Scalable off-chain channels supporting fair and atomic
cross-chain operations,''
\textit{IEEE Transactions on Computers}, vol.~72, no.~10, pp.~1--14, 2023.
\newblock DOI: \url{https://doi.org/10.1109/TC.2023.3288765}

%% ============================================================
%% SECTION: ZK PROOF SYSTEMS & AGGREGATION
%% ============================================================

\bibitem{bowe2019halo}
S.~Bowe, J.~Grigg, and D.~Hopwood,
``Recursive proof composition without a trusted setup,''
\textit{IACR ePrint Archive}, Report 2019/1021, 2019.
\newblock URL: \url{https://eprint.iacr.org/2019/1021}

\bibitem{kothapalli2022nova}
A.~Kothapalli, S.~Setty, and I.~Tzialla,
``Nova: Recursive zero-knowledge arguments from folding schemes,''
in \textit{Proc. Annual Int. Cryptology Conf. (CRYPTO)}, 2022,
pp.~359--388.
\newblock DOI: \url{https://doi.org/10.1007/978-3-031-15985-5_13}
\newblock ePrint: \url{https://eprint.iacr.org/2021/370}

\bibitem{plonky2}
Polygon~Zero~Team,
``Plonky2: Fast recursive arguments with PLONK and FRI,''
\textit{Technical Report}, 2022.
\newblock URL: \url{https://docs.rs/crate/plonky2/latest/source/plonky2.pdf}
...
\textit{IACR ePrint Archive}, Report 2019/953, 2019.
\newblock URL: \url{https://eprint.iacr.org/2019/953}

\bibitem{bunz2021snarkpack}
B.~B\"unz, M.~Maller, P.~Mishra, N.~Tyagi, and P.~Vesely,
``Proofs for inner pairing products and applications,''
in \textit{Proc. Int. Conf. Theory and Application of Cryptology
(ASIACRYPT)}, 2021, pp.~65--97.
\newblock  \url{https://link.springer.com/chapter/10.1007/978-3-030-92078-4_3}

\bibitem{chiesa2020marlin}
A.~Chiesa, Y.~Hu, M.~Maller, P.~Mishra, N.~Vesely, and N.~Ward,
``Marlin: Preprocessing zkSNARKs with universal and updatable SRS,''
in \textit{Proc. Annual Int. Conf. Theory and Applications of Cryptographic
Techniques (EUROCRYPT)}, 2020, pp.~738--768.
\newblock DOI: \url{https://doi.org/10.1007/978-3-030-45721-1_26}
\newblock ePrint: \url{https://eprint.iacr.org/2019/1047}

\bibitem{poseidon}
L.~Grassi, D.~Khovratovich, C.~Rechberger, A.~Roy, and M.~Schofnegger,
``Poseidon: A new hash function for zero-knowledge proof systems,''
in \textit{Proc. 30th USENIX Security Symp.}, 2021, pp.~519--535.
\newblock URL: \url{https://www.usenix.org/conference/usenixsecurity21/presentation/grassi}
\newblock ePrint: \url{https://eprint.iacr.org/2019/458}

%% ============================================================
%% SECTION: REPUTATION & CROSS-CHAIN TRUST
%% ============================================================

\bibitem{hu2024reputation}
X.~Hu, Y.~Ling, J.~Hua, Z.~Dong, and Y.~Sun,
``A blockchain cross-chain transaction method based on decentralized dynamic
reputation value assessment,''
\textit{IEEE Transactions on Services Computing}, vol.~17, no.~5,
pp.~2543--2556, 2024.
\newblock DOI: \url{https://doi.org/10.1109/TNSM.2024.3433414}

\bibitem{xiong2025reputation}
R.~Xiong, J.~Cheng, J.~Pu, X.~Dong, C.~Chen, and Z.~Xu,
``Enhancing trust and collaboration: A reputation-driven mechanism for
cross-chain IoT data sharing,''
\textit{Computer Networks}, vol.~261, 2025.
\newblock DOI: \url{https://doi.org/10.1016/j.comnet.2025.111266}

\bibitem{yuan2024doublelayer}
H.~Yuan, F.~Li, R.~Diao, and T.~Shu,
``Double-layer Byzantine fault-tolerant grouping consensus algorithm based on
Raft,''
\textit{IET Blockchain}, vol.~4, no.~1, pp.~42--52, 2024.
\newblock DOI: \url{https://doi.org/10.1049/blc2.12073}

%% ============================================================
%% SECTION: BFT CONSENSUS & DISTRIBUTED SYSTEMS
%% ============================================================

\bibitem{castro1999pbft}
M.~Castro and B.~Liskov,
``Practical Byzantine fault tolerance,''
in \textit{Proc. 3rd USENIX Symp. Operating Systems Design and Implementation
(OSDI)}, 1999, pp.~173--186.
\newblock URL: \url{https://dl.acm.org/doi/10.5555/296806.296824}

\bibitem{yin2019hotstuff}
M.~Yin, D.~Malkhi, M.~K.~Reiter, G.~G.~Gueta, and I.~Abraham,
``HotStuff: BFT consensus with linearity and responsiveness,''
in \textit{Proc. ACM Symp. Principles of Distributed Computing (PODC)}, 2019,
pp.~347--356.
\newblock DOI: \url{https://doi.org/10.1145/3293611.3331591}

\bibitem{lamport1982byzantine}
L.~Lamport, R.~Shostak, and M.~Pease,
``The Byzantine generals problem,''
\textit{ACM Trans. Programming Languages and Systems}, vol.~4, no.~3,
pp.~382--401, 1982.
...
\textit{Journal of the ACM}, vol.~35, no.~2, pp.~288--323, 1988.
\newblock DOI: \url{https://doi.org/10.1145/42282.42283}

\bibitem{fischer1985impossibility}
M.~J.~Fischer, N.~A.~Lynch, and M.~S.~Paterson,
``Impossibility of distributed consensus with one faulty process,''
\textit{Journal of the ACM}, vol.~32, no.~2, pp.~374--382, 1985.
\newblock DOI: \url{https://doi.org/10.1145/3149.214121}

\bibitem{miller2016honeybadger}
A.~Miller, Y.~Xia, K.~Croman, E.~Shi, and D.~Song,
``The Honey Badger of BFT protocols,''
in \textit{Proc. ACM SIGSAC Conf. Computer and Communications Security
(CCS)}, 2016, pp.~31--42.
\newblock DOI: \url{https://doi.org/10.1145/2976749.2978399}

%% ============================================================
%% SECTION: PROOF-OF-STAKE & CONSENSUS PROTOCOLS
%% ============================================================

\bibitem{chen2017algorand}
J.~Chen and S.~Micali,
``Algorand: A secure and efficient distributed ledger,''
\textit{Theoretical Computer Science}, vol.~777, pp.~155--183, 2019.
\newblock DOI: \url{https://doi.org/10.1016/j.tcs.2019.02.001}

\bibitem{kwon2014tendermint}
J.~Kwon,
``Tendermint: Consensus without mining,''
\textit{Draft v0.6}, 2014.
\newblock URL: \url{https://tendermint.com/static/docs/tendermint.pdf}

\bibitem{buterin2017casper}
V.~Buterin and V.~Griffith,
``Casper the Friendly Finality Gadget,''
\textit{arXiv preprint arXiv:1710.09437}, 2019.
\newblock URL: \url{https://arxiv.org/abs/1710.09437}

\bibitem{kiayias2017ouroboros}
A.~Kiayias, A.~Russell, B.~David, and R.~Oliynykov,
``Ouroboros: A provably secure proof-of-stake blockchain protocol,''
in \textit{Proc. Annual Int. Cryptology Conf. (CRYPTO)}, 2017,
pp.~357--388.
\newblock DOI: \url{https://doi.org/10.1007/978-3-319-63688-7_12}
\newblock ePrint: \url{https://eprint.iacr.org/2016/889}

%% ============================================================
%% SECTION: RANDOMNESS BEACONS & VRF
%% ============================================================

\bibitem{boneh2018verifiabledelay}
D.~Boneh, B.~B\"unz, and B.~Fisch,
``A survey of two verifiable delay functions,''
\textit{IACR ePrint Archive}, Report 2018/712, 2018.
\newblock URL: \url{https://eprint.iacr.org/2018/712}

\bibitem{drand}
S.~K.~Gorbunov, T.~Hanke, and others,
``Drand: A distributed randomness beacon,''
\textit{League of Entropy}, 2020.
\newblock URL: \url{https://drand.love}

%% ============================================================
%% SECTION: INCIDENT REPORTS & INDUSTRY DATA
%% \============================================================

\bibitem{crosschainbridgehack2023}
...
\textit{arXiv:2501.03423}, 2025.
\newblock DOI: \url{https://doi.org/10.48550/arXiv.2501.03423}

\bibitem{ronin2022postmortem}
Sky~Mavis,
``Community alert: Ronin Validators Compromised,''
\textit{Ronin Network Post-mortem}, March 29, 2022.
\newblock URL: \url{https://www.roninchain.com/blog/posts/community-alert-ronin-validators-6513cc78a5edc1001b03c366}

\bibitem{wormhole2022postmortem}
Wormhole~Foundation,
``Wormhole incident report,''
\textit{Wormhole Bridge Post-mortem}, February 5, 2022.
\newblock URL: \url{https://wormholecrypto.medium.com/wormhole-incident-report-02-02-22-ad9b8f21eec6}
