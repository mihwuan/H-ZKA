...
\begin{enumerate}
    \item We propose the Multi-Factor Proof-of-Participation (MF-PoP) reputation system, a multi-dimensional scoring mechanism that combines consistency, historical reliability, and liveness indicators with adaptive decay and progressive taxation. We give a closed-form isolation bound $T_{\mathrm{isolate}} \le \lceil \ln(R_0/R_{\min})/\ln(1/(1-\lambda\beta_{\min})) \rceil$ that holds for any admissible attacker (including oscillating and adaptive strategies), evaluates to $50$ rounds under our default parameters, and is verified empirically at $46.3 \pm 2.4$ rounds (30 seeds, jitter-injected). The bound replaces the honest committer assumption of zkCross with Byzantine resilience under the standard one-third Byzantine bound inherited from PBFT~\cite{castro1999pbft} and HotStuff~\cite{yin2019hotstuff}.
...
\end{equation}
Otherwise, the dampened update rule is applied:
\begin{equation}
...
A critical vulnerability in hierarchical architectures is that the cluster head ($\text{CH}_m$) is responsible for scoring its committers ($C_i^t$). A malicious cluster head could intentionally score a valid proof as invalid ($C_i^t = 0$) to arbitrarily slash an honest committer. To mitigate this risk, H-ZKA implements an on-chain arbitration mechanism.
...
\begin{proof}
During the valid rounds ($C_i^t = 1$), the reputation increases slowly due to the base $\beta = 0.08$. However, during the invalid round ($C_i^t = 0$), the penalty multiplier (5x) causes $\beta_i^t = 0.40$, dropping the reputation significantly. Because $0.40 \gg 5 \times 0.08$, the net reputation change over the 6-round cycle is strictly negative. Thus, $R_i^t$ decreases monotonically across cycles until the Trust Jail threshold ($1.5 \cdot R_{\min}$) is breached, permanently isolating the node. Furthermore, each invalid round burns 10\% of the remaining stake, making the attack economically unsustainable.
\end{proof}
...
\Repeat
  \For{each chain $j\in\{1,\ldots,k\}$}
    \State Assign $j$ to $\mathcal{C}_{\arg\min_m \mathbf{D}[j,\mu_m]}$
  \EndFor
  \For{$m=1$ to $M$}
    \State $\mu_m \gets \arg\min_{j\in\mathcal{C}_m} \sum_{j'\in\mathcal{C}_m}\mathbf{D}[j,j']$
  \EndFor
\Until medoids stable or $20$ iterations elapsed
\State Rebalance any cluster with $|\mathcal{C}_m|>2\lceil\sqrt{k}\rceil$ by deterministic VRF-tiebreak
...
To achieve sub-linear scalability, we introduce a recursive aggregation circuit, $\aggCircuit$. At each round $t$, cluster heads in $\mathcal{C}_m$ collect individual proofs $\{\pi_j\}_{j \in \mathcal{C}_m}$ and generate a succinct meta-proof.
...
\begin{algorithmic}[1]
\Require 
    \Statex Individual proofs $\{\pi_j\}_{j \in \mathcal{C}_m}$ and verification keys $\{\vk_j\}$
    \Statex State transitions $\{\rootOld, \rootNew\}_{j \in \mathcal{C}_m}$ and transaction batches $\{\mathsf{tx}_j\}$
\Ensure 
    \Statex Aggregated proof $\pi_{\text{agg}}$ for cluster $\mathcal{C}_m$

\State $(\pk_{\text{agg}}, \vk_{\text{agg}}) \gets \Pi.\mathsf{Setup}(1^\lambda, \aggCircuit)$ \Comment{Pre-computed}
\State $\vec{x} \gets \{ \rootNew \}_{j \in \mathcal{C}_m} \parallel \mathsf{id}_m \parallel \mathsf{round}_t$ \Comment{Public Inputs}
\State $\vec{w} \gets \{ \pi_j, \rootOld, \mathsf{tx}_j \}_{j \in \mathcal{C}_m}$ \Comment{Private Witnesses}

\For{each chain $j \in \mathcal{C}_m$} \label{alg:loop}
...
\State $\mathsf{TxClusterCommit} \gets \left( \mathsf{CH}_m, \{\rootNew\}, \pi_{\text{agg}}, \sigma_{\mathsf{CH}_m} \right)$
...
The complete H-ZKA protocol proceeds in rounds. First, committers ($\text{CT}_i$) on each chain collect the latest transaction batch, generate a zk-SNARK proof $\pi_j$ via the $\Psi.\text{Commit}$ protocol (identical to the original zkCross), and submit $\text{TxCommit}$ to their regional cluster $\mathcal{C}_m$. Second, the cluster head $\text{CH}_m$ receives proofs from all chains in $\mathcal{C}_m$, cross-references the submitted state roots against each other for consistency, computes the consistency score $C_i^t$ for each committer, and triggers reputation updates via the MF-PoP mechanism. Third, $\text{CH}_m$ runs the aggregation circuit $\aggCircuit$ (Algorithm~\ref{alg:aggregation}) to produce $\pi_{\text{agg}}$ and submits $\text{TxClusterCommit}$ to the global audit chain. Fourth, the global audit chain verifies the $M$ aggregated proofs, and upon consensus by $\geq 2/3$ of the aggregated submissions, the global state root $\text{root}_{\text{global}}$ is updated. Fifth, cluster head reputation is updated at the global level; if $R_{\text{CH}} < R_{\text{median}}$ or a VRF-triggered rotation event occurs, a new cluster head is elected.
...
H-ZKA preserves the unlinkability property of protocols $\Theta$ and $\Phi$, i.e., for all polynomial-time adversaries $\mathcal{A}$:
\begin{equation}
...
\label{thm:byzantine}
H-ZKA tolerates up to $f < |\mathcal{C}_m|/3$ Byzantine committers per cluster without compromising the integrity of auditing results, and Byzantine committers are isolated to $R_{\min}$ within 46 rounds.
\end{theorem}
