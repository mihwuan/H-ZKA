...
\title{zkCross v2: Byzantine-Resilient Reputation and Hierarchical Proof Aggregation for Scalable Cross-Chain Privacy-Preserving Auditing}
...
\begin{abstract}
Cross-chain interoperability has become a critical requirement in the multi-chain blockchain ecosystem. While recent work such as zkCross introduced a two-layer architecture that decouples privacy-preserving cross-chain transactions from auditing via zk-SNARKs, two significant limitations remain: (1) the reliance on an honest committer assumption, which is vulnerable to Byzantine failures, and (2) the linear scaling of audit workload with the number of chains, which limits applicability to large-scale networks. This paper proposes zkCross v2, an enhanced cross-chain privacy-preserving auditing framework that addresses both shortcomings through two synergistic mechanisms. First, we introduce a Multi-Factor Proof-of-Participation (MF-PoP) reputation system that replaces the honest committer assumption with an adaptive, multi-dimensional scoring scheme capable of autonomously isolating Byzantine committers within approximately 48 auditing rounds. Second, we design a three-layer hierarchical clustering architecture with recursive zk-SNARK proof aggregation, reducing the number of proofs submitted to the global audit chain from $O(k)$ to $O(\sqrt{k})$, where $k$ is the number of ordinary chains. The two mechanisms reinforce each other: reputation scores drive cluster head election via quadratic weighting and verifiable random functions, while cluster-level cross-referencing strengthens reputation accuracy. Crucially, both enhancements operate exclusively on the auditing layer and leave the privacy-preserving transfer and exchange protocols entirely unchanged, preserving the unlinkability guarantees of the original design. Theoretical analysis demonstrates that zkCross v2 achieves Byzantine fault tolerance equivalent to $f < n/3$ without trusted third parties, and complexity analysis confirms the $\sqrt{k}$-factor reduction in audit workload. Simulation results validate the effectiveness of the proposed mechanisms across networks of up to 200 chains.
\end{abstract}
...
This paper proposes zkCross v2, which enhances the original zkCross architecture with two synergistic mechanisms that operate exclusively on the auditing layer, leaving the privacy-preserving transfer protocol $\Theta$ and exchange protocol $\Phi$ entirely unchanged. The main contributions of this paper are as follows:
\begin{enumerate}
    \item We propose the Multi-Factor Proof-of-Participation (MF-PoP) reputation system, a multi-dimensional scoring mechanism that combines consistency, historical reliability, and liveness indicators with adaptive decay and progressive taxation to autonomously isolate Byzantine committers within approximately 46 auditing rounds, replacing the honest committer assumption with Byzantine fault tolerance at the $f < n/3$ threshold.
    \item We introduce a three-layer hierarchical clustering architecture with recursive zk-SNARK proof aggregation, where ordinary chains are grouped into $\sqrt{k}$ regional clusters, each managed by a reputation-elected cluster head that aggregates intra-cluster proofs using a recursive verification circuit $\Lambda_{\Psi\_agg}$, reducing the number of proofs on the global audit chain from $O(k)$ to $O(\sqrt{k})$ while preserving the constant proof size of 127 bytes.
    \item We provide a formal security analysis demonstrating that zkCross v2 preserves the unlinkability properties of the original protocols, achieves the claimed Byzantine resilience, and delivers the $\sqrt{k}$-factor audit workload reduction. Simulation results across networks of up to 200 chains validate the theoretical predictions.
\end{enumerate}
...
zkCross v2 (Ours) & \checkmark & \checkmark & \checkmark & \checkmark & $O(\sqrt{k} \times m)$ \\
\bottomrule
\end{tabular}
}
\end{table}

As shown in Table~\ref{tab:comparison}, no existing approach simultaneously provides privacy-preserving cross-chain protocols, integrated auditing, Byzantine resilience for the auditing pipeline, and proof aggregation for audit scalability. zkCross v2 fills this gap.
...
$\mathcal{C}_m$ & The $m$-th regional cluster, $m \in [1, M]$ \\
$\text{CH}_m$ & Cluster head of $\mathcal{C}_m$ \\
...
$\Lambda_{\Psi\_\text{agg}}$ & Recursive aggregation circuit \\
...
We inherit the network model of zkCross~\cite{guo2024zkcross} and extend it with a three-layer architecture. The system consists of the following entities. \textit{Ordinary chains} ($\text{Chain}_1, \text{Chain}_2, \ldots, \text{Chain}_k$) are independent blockchains where cross-chain transactions (transfers and exchanges) occur, each maintaining its own consensus and state. \textit{Committers} ($\text{CT}_i$) are nodes responsible for generating zk-SNARK proofs that attest to the correctness of state transitions within their assigned chain; unlike the original zkCross, committers in zkCross v2 are not assumed to be honest. \textit{Regional clusters} ($\mathcal{C}_1, \mathcal{C}_2, \ldots, \mathcal{C}_M$) are groups of approximately $\sqrt{k}$ chains, each managed by a cluster head $\text{CH}_m$ elected through reputation-weighted verifiable random functions. The \textit{global audit chain} is the top-layer chain where aggregated proofs from cluster heads are verified by auditors, maintaining a global state root $\text{root}_{\text{global}} = \text{MerkleTree}(\{\text{cluster\_root}_m\}_{m=1}^{M})$. Finally, \textit{auditors} ($\text{AD}$) are nodes on the global audit chain that verify aggregated proofs and maintain the global audit state.

\begin{figure}[t]
    \centering
    % Thay 'architecture-v2.png' bằng tên file ảnh bạn đã lưu
    \includegraphics[width=0.5\textwidth]{architecture.jpg} 
    \caption{The three-layer architecture of zkCross v2. Layer 0 consists of ordinary chains grouped into regional clusters. Layer 1 contains cluster heads that aggregate proofs within each cluster. Layer 2 is the global audit chain that verifies $O(\sqrt{k})$ aggregated proofs per round.}
    \label{fig:architecture}
\end{figure}

\subsection{Threat Model}

We adopt a stronger threat model than the original zkCross to reflect realistic multi-chain environments. Committers may exhibit Byzantine behavior, including submitting invalid proofs, withholding proofs, or colluding with other malicious committers; we assume that the fraction of Byzantine committers within any cluster satisfies $f < |\mathcal{C}_m|/3$. Cluster heads may also behave maliciously, but the reputation-weighted election mechanism and VRF-based rotation ensure that Byzantine cluster heads are replaced within a bounded number of rounds. The underlying zk-SNARK proof system (Groth16) is assumed to satisfy completeness, soundness, and zero-knowledge properties as established by Groth~\cite{groth2016size}. The collision-resistant hash function used in the protocols is assumed to be secure against polynomial-time adversaries. Network communication is asynchronous with eventual delivery, meaning that adversaries may delay but not permanently suppress messages.

\subsection{Design Goals}

Given the threat model, zkCross v2 aims to achieve the following goals. (1)~\textit{Privacy preservation}: the unlinkability properties of the transfer protocol $\Theta$ and exchange protocol $\Phi$ must remain intact, i.e., within an anonymity set, the adversary cannot link a sender to a receiver. (2)~\textit{Byzantine resilience}: the system must autonomously detect and isolate malicious committers without requiring trusted third parties, achieving fault tolerance at the $f < n/3$ threshold. (3)~\textit{Scalable auditing}: the number of proofs that auditors on the global chain must verify per round should be $O(\sqrt{k})$ instead of $O(k)$. (4)~\textit{Proof succinctness}: the aggregated proof size must remain constant (127 bytes) regardless of the number of aggregated individual proofs.

\subsection{Problem Definition}

\begin{definition}[Byzantine-Resilient Cross-Chain Auditing]
Given a network of $k$ ordinary blockchains, each producing $m$ blocks per second with $n$ transactions per block, and a set of committers $\{\text{CT}_i\}$ where up to $f < |\mathcal{C}_m|/3$ committers per cluster may be Byzantine, design an auditing architecture that:
\begin{enumerate}
    ...
    \item Preserves the unlinkability of cross-chain transfer and exchange protocols, i.e., $|\Pr[\text{Exp}^{\text{Unlinkability}}_{\mathcal{A}, \text{zkCross v2}}(\lambda) = 1] - \frac{1}{2}| \leq \text{negl}(\lambda)$.
\end{enumerate}
\end{definition}
...
\subsubsection{Consistency Score $C_i^t$}
...
\subsubsection{Historical Reliability Score $H_i^t$}
...
\subsubsection{Liveness Indicator $L_i^t$}
...
\subsection{Adaptive Decay and Reputation Update}

The reputation update rule uses a fixed base decay rate $\beta = 0.08$ and a non-linear slashing mechanism. This ensures that a Byzantine committer is isolated within approximately 48 rounds:
\begin{equation}
\beta = 0.08
\label{eq:beta}
\end{equation}

For a committer submitting invalid proofs ($C_i^t = 0$), a slashing multiplier is applied:
\begin{equation}
R_i^t = R_i^{t-1} \cdot 0.5
\label{eq:slashing}
\end{equation}

To prevent a compromised node from recovering via the oscillating attack, we introduce a \textit{Trust Jail} mechanism. If a node's reputation falls below a threshold near $R_{\min}$ ($1.5 \cdot R_{\min}$), it is frozen:
\begin{equation}
R_i^t = 
\begin{cases} 
R_i^{t-1} & \text{if } R_i^{t-1} \leq 1.5 \cdot R_{\min} \\
\text{clamp}\left((1 - \beta) \cdot R_i^{t-1} + \beta \cdot Q_i^t, \ 0.01, \ 10.0\right) & \text{otherwise}
\end{cases}
...
To prevent reputation centralization, zkCross v2 employs a progressive taxation mechanism for high-reputation nodes:
\begin{equation}
...
\begin{equation}
\text{register}(\text{CT}_{\text{new}}) \Leftarrow \text{PoW}(\text{CT}_{\text{new}}) \land R_{\text{endorser}_1} \geq 0.7 \land R_{\text{endorser}_2} \geq 0.7.
\label{eq:onboarding}
\end{equation}
...
\label{thm:isolation}
A consistently Byzantine committer with initial reputation $R_0 = 0.5$ is reduced to $R_{\min} = 0.01$ within approximately 48 auditing rounds under the MF-PoP mechanism.
\end{theorem}

\begin{proof}
Consider a Byzantine committer that consistently submits invalid proofs. With the non-linear slashing mechanism, its reputation is halved upon each detection (Eq.~\ref{eq:slashing}). Starting from $R_0 = 0.5$, the reputation after $n$ detections is $R_n = 0.5 \cdot (0.5)^n$. 
For a consistent attacker, $n$ increases each round. Reaching $R_{\min} = 0.01$ requires $(0.5)^n \leq 0.02$, which occurs at $n = \lceil \log_{0.5}(0.02) \rceil = 6$ detections.
However, empirical simulations incorporating network latency, the multi-factor scoring (Eq.~\ref{eq:quality}), and the fixed base decay $\beta = 0.08$ (Eq.~\ref{eq:beta}), show the effective isolation boundary is reached at mean $t = 48.0 \pm 0.0$ rounds. Once $R_i \leq 1.5 \cdot R_{\min}$, the Trust Jail mechanism freezes the reputation, isolating the node.
\end{proof}
...
\begin{equation}
\bigcup_{m=1}^{M} \mathcal{C}_m = \{\text{Chain}_1, \ldots, \text{Chain}_k\}, \quad \mathcal{C}_i \cap \mathcal{C}_j = \emptyset \text{ for } i \neq j.
\label{eq:partition}
\end{equation}

\subsection{Cluster Head Election}

Within each cluster $\mathcal{C}_m$, the cluster head $\text{CH}_m$ is elected using reputation-quadratic weighting combined with a Verifiable Random Function (VRF) to ensure both meritocratic selection and unpredictability:

\begin{equation}
w_i = (R_i)^2, \quad \text{totalWeight} = \sum_{i \in \mathcal{C}_m} w_i
\label{eq:weight}
\end{equation}
\begin{equation}
...
\end{equation}
\begin{equation}
\text{CH}_m = \text{weightedSelect}(\mathcal{C}_m, \text{seed} \mod \text{totalWeight})
\label{eq:election}
\end{equation}
...
To prevent long-term collusion among chains assigned to the same cluster, zkCross v2 performs a global VRF-based cluster reassignment every $E_{\text{epoch}} = 100$ rounds. During each epoch boundary:
\begin{equation}
\text{shuffled\_chains} = \text{VRF}\text{-}\text{shuffle}(\{1, \ldots, k\}, \text{seed}_{\text{epoch}})
\label{eq:epoch_shuffle}
\end{equation}
The $k$ chains are randomly redistributed among the $M$ clusters using a verifiable random permutation seeded by a global random beacon (e.g., block hash). This prevents an attacker from: (1) static cluster targeting by colluding with a specific subset of chains, (2) cluster head persistence by maintaining influence within a fixed group, and (3) predictable re-elections by gaming the cluster head selection. Upon reassignment, all cluster heads are re-elected using the reputation-weighted VRF mechanism (Eq.~\ref{eq:election}), ensuring that only high-reputation nodes can lead clusters in the next epoch. This combination of temporal cluster reassignment and reputation-based head election provides strong Byzantine resilience against coordinated attacks across epochs.

\subsection{Intra-Cluster Proof Aggregation}

The core scalability improvement comes from the recursive aggregation circuit $\Lambda_{\Psi\_\text{agg}}$. When committers in cluster $\mathcal{C}_m$ submit their individual proofs $\{\pi_j\}_{j \in \mathcal{C}_m}$ for the current round, the cluster head constructs an aggregated proof as follows.

\begin{algorithm}[t]
\caption{Intra-Cluster Proof Aggregation}
\label{alg:aggregation}
\begin{algorithmic}[1]
\REQUIRE Individual proofs $\{\pi_j\}_{j \in \mathcal{C}_m}$, verification keys $\{\text{vk}_j\}$, state roots $\{\text{root}_{\text{old}_j}, \text{root}_{\text{new}_j}\}$
\ENSURE Aggregated proof $\pi_{\text{agg}}$ for cluster $\mathcal{C}_m$
\STATE $(\text{pk}_{\text{agg}}, \text{vk}_{\text{agg}}) \leftarrow \Pi.\text{Setup}(1^\lambda, \Lambda_{\Psi\_\text{agg}})$
\STATE Set public inputs $\vec{x} \leftarrow \{\text{root}_{\text{new}_j}\}_{j \in \mathcal{C}_m} \| \text{cluster\_id} \| \text{round}_t$
\STATE Set private inputs $\vec{w} \leftarrow \{\pi_j, \text{root}_{\text{old}_j}, \text{tx\_batch}_j\}_{j \in \mathcal{C}_m}$
\FOR{each chain $j \in \mathcal{C}_m$}
    \STATE Verify: $\Pi.\text{Verify}(\text{vk}_j, \vec{x}_j, \pi_j) \stackrel{?}{=} 1$
    \STATE Verify: $\text{stateTransition}(\text{root}_{\text{old}_j}, \text{tx\_batch}_j) \stackrel{?}{=} \text{root}_{\text{new}_j}$
\ENDFOR
\STATE $\pi_{\text{agg}} \leftarrow \Pi.\text{Prove}(\text{pk}_{\text{agg}}, \vec{x}, \vec{w})$
\STATE Construct $\text{TxClusterCommit} = (\text{From}: \text{CH}_m; \{\text{root}_{\text{new}_j}\}; \pi_{\text{agg}}; \sigma_{\text{CH}_m})$
\STATE Submit $\text{TxClusterCommit}$ to the global audit chain
\end{algorithmic}
\end{algorithm}

The circuit $\Lambda_{\Psi\_\text{agg}}$ performs two operations for each chain in the cluster: (1) recursive verification of the individual proof $\pi_j$ using the corresponding verification key, and (2) consistency checking that the claimed state transition matches the proof. The output is a single aggregated proof $\pi_{\text{agg}}$ that attests to the validity of all state transitions within the cluster.

\subsection{Global Audit Chain Verification}

The global audit chain receives $M = \lceil \sqrt{k} \rceil$ aggregated proofs per round, one from each cluster head. Auditors verify each $\pi_{\text{agg}}$ in constant time $O(1)$ due to the succinctness of zk-SNARKs, yielding a total verification workload of:
\begin{equation}
W_{\text{audit}} = O(M) = O(\sqrt{k})
\label{eq:audit_workload}
\end{equation}
compared to $O(k)$ in the original zkCross. Upon successful verification of at least $\lceil 2M/3 \rceil$ cluster submissions, the global state root is updated:
\begin{equation}
\text{root}_{\text{global}} = \text{MerkleTree}\left(\{\text{cluster\_root}_m\}_{m=1}^{M}\right)
\label{eq:global_root}
\end{equation}
where $\text{cluster\_root}_m = \text{MerkleTree}\left(\{\text{root}_{\text{new}_j}\}_{j \in \mathcal{C}_m}\right)$.

\subsection{Data Availability and Fraud Proof Mechanism}

A critical vulnerability identified in early designs of zkCross v2 is the potential for malicious cluster heads to censor data by withholding proofs from certain chains within their cluster. Although the global audit chain employs Byzantine fault tolerance, the disjoint nature of cluster data---where each cluster holds only its assigned chains' proofs---could theoretically allow a compromised cluster head to omit a subset of chains without detection, as long as the remaining clusters' proofs still meet the $\lceil 2M/3 \rceil$ threshold.

To address this risk, zkCross v2 implements a Data Availability (DA) challenge mechanism with a bounded challenge window and fraud proofs. The mechanism operates as follows:
...
The fraud proof submitted in the challenge includes: (1) cluster and chain identification (cluster ID $m$ and chain ID $j$), (2) round metadata (the round $t$ and the challenge filing time), (3) state evidence (a cryptographic proof that chain $j$ produced valid state transitions during round $t$, attested by $\geq 2/3$ of the chain's validators), and (4) omission attestation (evidence that $\text{CH}_m$ did not include proof for chain $j$ in its aggregated submission).

\subsubsection{Challenge Resolution}
Upon receiving a fraud proof: (1) the global audit chain verifies the proof validity using standard consensus mechanisms (e.g., majority vote among auditors); (2) if valid, the cluster head $\text{CH}_m$ reputation is immediately set to $R_{\min} = 0.01$, a new cluster head is immediately elected via the VRF mechanism (Section~V.2), and the missing chain data is recovered and incorporated into the global state; (3) if invalid (the challenge is frivolous), the challenger's reputation is reduced by $\beta = 0.08$.
...
Under the DA challenge mechanism, if a cluster head $\text{CH}_m$ censors the proof of any chain $j \in \mathcal{C}_m$, an honest node can file a valid fraud proof within $T_{\text{challenge}} = 10$ rounds, causing $\text{CH}_m$ to be isolated to $R_{\min}$. Thus, sustained data censorship is prevented with probability approaching 1 as the number of honest nodes grows.
\end{theorem}
...
zkCross v2 reduces the per-round audit verification complexity from $O(k \times m)$ to $O(\sqrt{k} \times m)$, representing a $\sqrt{k}$-factor improvement.
\end{theorem}
...
The complete zkCross v2 protocol proceeds in rounds. First, committers ($\text{CT}_i$) on each chain collect the latest transaction batch, generate a zk-SNARK proof $\pi_j$ via the $\Psi.\text{Commit}$ protocol (identical to the original zkCross), and submit $\text{TxCommit}$ to their regional cluster $\mathcal{C}_m$. Second, the cluster head $\text{CH}_m$ receives proofs from all chains in $\mathcal{C}_m$, cross-references the submitted state roots against each other for consistency, computes the consistency score $C_i^t$ for each committer, and triggers reputation updates via the MF-PoP mechanism. Third, $\text{CH}_m$ runs the aggregation circuit $\Lambda_{\Psi\_\text{agg}}$ (Algorithm~\ref{alg:aggregation}) to produce $\pi_{\text{agg}}$ and submits $\text{TxClusterCommit}$ to the global audit chain. Fourth, the global audit chain verifies the $M$ aggregated proofs, and upon consensus by $\geq 2/3$ of the aggregated submissions, the global state root $\text{root}_{\text{global}}$ is updated. Fifth, cluster head reputation is updated at the global level; if $R_{\text{CH}} < R_{\text{median}}$ or a VRF-triggered rotation event occurs, a new cluster head is elected.
...
zkCross v2 preserves the unlinkability property of protocols $\Theta$ and $\Phi$, i.e., for all polynomial-time adversaries $\mathcal{A}$:
\begin{equation}
\left|\Pr\left[\text{Exp}^{\text{Unlinkability}}_{\mathcal{A}, \text{zkCross v2}}(\lambda) = 1\right] - \frac{1}{2}\right| \leq \text{negl}(\lambda).
\end{equation}
\end{theorem}

\begin{proof}
The components introduced in zkCross v2 (MF-PoP reputation, hierarchical clustering, proof aggregation) interact only with the auditing protocol $\Psi$. The circuits $\Lambda_\Theta$ (Figure~3 in~\cite{guo2024zkcross}), $\Lambda_\Phi^{\text{off}}$ (Figure~4), and $\Lambda_\Phi^{\text{on}}$ (Figure~5) are not modified. The private inputs (sender/receiver public keys, random numbers, serial numbers, preimages) and the zero-knowledge proofs generated by these circuits remain identical to the original zkCross. Since the proof of Theorem~1 in~\cite{guo2024zkcross} depends solely on the zero-knowledge property of zk-SNARK, the collision resistance of the hash function, and the circuit design of $\Theta$ and $\Phi$, and none of these are altered, the unlinkability guarantee carries over to zkCross v2.
\end{proof}
...
\label{thm:byzantine}
zkCross v2 tolerates up to $f < |\mathcal{C}_m|/3$ Byzantine committers per cluster without compromising the integrity of auditing results, and Byzantine committers are isolated to $R_{\min}$ within 46 rounds.
\end{theorem}

\begin{proof}
The argument proceeds in two parts. First, for audit integrity: the aggregation circuit $\Lambda_{\Psi\_\text{agg}}$ internally verifies each individual proof $\pi_j$. If a Byzantine committer submits an invalid proof, the verification step in line 5 of Algorithm~\ref{alg:aggregation} fails, and the proof is excluded from the aggregation. The cluster head, elected by reputation weighting, processes only valid proofs. If the cluster head itself is Byzantine, it may produce an invalid $\pi_{\text{agg}}$; however, the global audit chain verifies $\pi_{\text{agg}}$ using $\Pi.\text{Verify}$, which rejects invalid aggregated proofs. Since the global chain requires agreement from $\geq 2/3$ cluster submissions, up to $\lfloor(M-1)/3\rfloor$ corrupted clusters cannot compromise the global audit state. Second, for isolation: by Theorem~\ref{thm:isolation}, consistently Byzantine committers are reduced to $R_{\min} = 0.01$ within 48 rounds. At $R_{\min}$, the node's weight in cluster head elections is $(0.01)^2 = 0.0001$, rendering it negligible compared to honest nodes with $R \geq 0.5$.
\end{proof}
...
We implement zkCross v2 as an extension of the original zkCross implementation. The off-chain zk-SNARK components are built using xjsnark with the Groth16 proof system. On-chain components are implemented using go-ethereum with Solidity smart contracts for the ReputationRegistry and ClusterManager. The aggregation circuit $\Lambda_{\Psi\_\text{agg}}$ is implemented using recursive proof composition.

% [SỬA LỖI B1] Ghi nhận số constraint thực tế cho recursive verification
\textbf{Constraint analysis (corrected):} Recursive Groth16 verification on non-native fields (e.g., BN254 pairing emulation) requires approximately 20 million constraints per recursive verification step, rather than the previously estimated 40,000 constraints. This 500$\times$ correction is consistent with benchmarks from gnark~\cite{rosenberg2024hekaton} and reflects the cost of emulating non-native field arithmetic within the circuit.

% [SỬA LỖI B4-S3] Micro-benchmarking thực tế
Experiments are conducted on cloud servers using 50 ecs.g6.3xlarge instances, each with Intel Xeon (Cascade Lake) Platinum 8269CY processor, 12 vCPUs at 3.2 GHz, and 48 GB RAM, running Ubuntu 20.04. Each instance runs 4 Docker nodes. We evaluate the system across network sizes ranging from 25 to 200 ordinary chains, with each chain processing up to 10,000 transactions.

% [SỬA LỖI B4-S1] Mô phỏng mạng heterogeneous
\textbf{Network conditions:} To simulate realistic heterogeneous network conditions, we inject variable latency (50--500ms), bandwidth limits (1--10 Mbps), and packet loss (1--5\%) into Docker containers using Linux Traffic Control (\texttt{tc/netem}). This approach is consistent with evaluation practices in prior cross-chain work~\cite{xie2022zkbridge, rosenberg2024hekaton}.
...
\caption{Reputation convergence trajectories from TN1 simulation. Byzantine committers (red) converge to $R_{\min}=0.01$ within $48.0 \pm 0.0$ rounds, while honest committers (blue) converge to high reputation. Simulation parameters: 100 committers, 30\% Byzantine, 100 rounds, 10 seeds.}
\label{fig:reputation}
\end{figure}

Fig~\ref{fig:reputation} demonstrates the reputation convergence behavior from our simulation with 100 committers (30% Byzantine) over 100 rounds. Starting from the initial reputation $R_0 = 0.5$, an honest committer (consistently producing valid proofs) converges to $R \approx 0.99$ after 50 rounds. A Byzantine committer (consistently submitting invalid proofs) is reduced to $R_{\min} = 0.01$ after $48.0 \pm 0.0$ rounds (mean $\pm$ std over 10 seeds), confirming Theorem~\ref*{thm:isolation}. The adaptive decay rate $\beta$ accelerates the descent for low-reputation nodes, providing a fast response to persistent misbehavior.
...
\caption{MF-PoP reputation trajectory under oscillating attack (5-correct/1-incorrect strategy). Honest committers (blue) converge to $R \approx 1.0$, while Byzantine attackers (red) are isolated to $R_{\min} = 0.01$ within 46 rounds. The fixed MF-PoP mechanism (green dashed line) prevents recovery attempts, ensuring sustained isolation despite oscillating behavior.}
...
\caption{Cumulative stake slashing over 200 rounds under the non-linear slashing mechanism (Eq.~\ref{eq:slashing}). Byzantine committers lose 96.9\% of their stake (solid red line) by round 200, while honest committers' stakes remain intact. This provides strong economic deterrence against oscillating attacks and prevents attackers from maintaining influence.}
...
\begin{table}[t]
\centering
\caption{Per-round audit verification counts: zkCross vs.\ zkCross v2.}
\label{tab:workload}
\begin{tabular}{rccc}
\toprule
$k$ (chains) & zkCross & zkCross v2 & Reduction Factor \\
\midrule
25  & 25  & 5  & $5.0\times$ \\
50  & 50  & 8  & $6.3\times$ \\
...
Table~\ref{tab:workload} shows the reduction in the number of proof verifications per auditing round. For $k = 100$ chains, the original zkCross requires 100 individual proof verifications, while zkCross v2 requires only 10 aggregated proof verifications (one per cluster), achieving a $10\times$ reduction that matches the theoretical $\sqrt{k} = 10$ prediction. At $k = 200$, the reduction factor increases to $13.3\times$, consistent with $\sqrt{200} \approx 14.1$.

\subsection{Latency and Throughput}

\begin{table}[t]
\centering
\caption{End-to-end audit latency comparison (seconds).}
\label{tab:latency}
\begin{tabular}{rcccc}
\toprule
$k$ & zkCross & \multicolumn{2}{c}{zkCross v2} & Speedup \\
    & Total   & Aggregation & Global Audit & \\
\midrule
25  & 10.00 & 0.15 & 1.50 & $6.1\times$ \\
50  & 20.00 & 0.24 & 2.40 & $7.6\times$ \\
100 & 40.00 & 0.51 & 2.80 & $12.1\times$ \\
150 & 60.00 & 0.65 & 3.64 & $14.0\times$ \\
200 & 80.00 & 0.76 & 4.20 & $16.1\times$ \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:latency} compares the end-to-end audit latency. The original zkCross exhibits linear growth: at $k = 100$, auditing takes approximately 40.0 seconds, consistent with the figures reported in the original paper. zkCross v2 introduces a two-phase audit: intra-cluster aggregation (0.15--0.76 seconds depending on cluster size) followed by global audit chain verification (1.50--4.20 seconds). The total latency grows sub-linearly with $k$, achieving $12.1\times$ speedup at $k = 100$ and $16.1\times$ at $k = 200$. The aggregation time grows slowly because each cluster processes approximately $\sqrt{k}$ proofs regardless of the total network size.
...
\begin{tabular}{lccc}
\toprule
Metric & zkCross & zkCross v2 & Reduction \\
\midrule
...
Table~\ref{tab:circuit} shows the corrected circuit constraints for the aggregation circuit. Each recursive verification requires approximately 20 million constraints due to non-native field arithmetic emulation for Groth16 pairing operations on BN254. This is 500$\times$ higher than previously estimated, and represents the actual cost of recursive zk-SNARK composition without cycle-of-curves optimization. For a cluster of 10 chains ($k = 100$), the total overhead is approximately 200 million constraints, requiring $\sim$200 seconds of prover time and $\sim$7.5 GB RAM on a single CPU core. While this overhead is substantial, it is amortized across $\sqrt{k}$ chains and occurs off-chain. For future work, adopting Halo2 or Nova-style folding schemes~\cite{rosenberg2024hekaton} could reduce this overhead by 2--3 orders of magnitude.
...
\caption{Audit accuracy under Byzantine committers ($k=100$), showing accuracy recovery over rounds.}
\label{tab:byzantine}
\begin{tabular}{lcccccc}
\toprule
...
This paper presents zkCross v2, an enhanced cross-chain privacy-preserving auditing framework that addresses the honest committer assumption and linear audit scalability limitations of the original zkCross. The Multi-Factor Proof-of-Participation reputation mechanism with non-linear slashing replaces the unrealistic honest committer assumption with an adaptive, multi-dimensional scoring system that isolates Byzantine committers within 48 rounds, achieving fault tolerance at the $f < n/3$ threshold without trusted third parties. We demonstrate that oscillating attackers (5-correct/1-wrong strategy) lose 96.9\% of their stake under the enhanced slashing mechanism, providing strong economic deterrence. The three-layer hierarchical clustering architecture with VRF-based epoch shuffling and recursive zk-SNARK proof aggregation reduces the audit workload from $O(k)$ to $O(\sqrt{k})$ while preserving constant proof size. We acknowledge that recursive Groth16 verification incurs approximately 20 million constraints per recursive step due to non-native field arithmetic, representing a significant computational overhead that future work should address through cycle-of-curves optimization or folding schemes. Both mechanisms operate on the auditing layer and leave the privacy-preserving transfer and exchange protocols unchanged, preserving unlinkability guarantees. Experimental results confirm a $10\times$ reduction in audit verification counts at $k = 100$ chains and a $12.1\times$ speedup in end-to-end audit latency.
